...
from reentry_options_sentiment_live import build_signal as build_options_sentiment
...
def vix_term_structure(market_date: str|None):
    ...
    return {"name":"Volatility curve repair","state":state,"supportive":cur<=1 or (prior>1 and cur<prior),"current_ratio":cur,"prior_ratio":prior,"change":cur-prior,"normalization_cross":prior>1 and cur<=1,"benchmark":"Near-term volatility divided by three-month volatility: above 1.10 = acute inversion · above 1.00 = stressed/inverted · move below 1.00 = normalization · below 0.97 = normal upward-sloping curve","retail_explanation":"This compares near-term market fear with three-month fear. A move back below 1.00 means immediate stress has normalized and is generally healthier for a rebound.","signal_behavior":"CONFIRMING","behavior_explanation":"This is mainly a confirmation signal: falling below 1.00 shows short-term volatility stress is normalizing. A high ratio itself is not bullish; the repair is what matters.","freshness_type":"DAILY_CLOSE","last_updated":last,"source":"Cboe official daily VIX and VIX3M history","validation_status":"VALIDATED_EVENT_SUPPORT","validation_note":"Frozen 2007-2026 falsification: Nasdaq-100 ETF 10-day return after the volatility curve normalized below 1.00, 78 events, median +2.17%, positive 69.2%, matched excess +1.17%; workflow run 33811926596."}
...
def main():
    ...
    try:
        ...
        t2108={"name":"NYSE Stocks Above 40-Day Moving Average","symbol":"NYSE-ABOVE-40DMA","state":"UNAVAILABLE","direction":"UNAVAILABLE","decision_input":False,"retail_explanation":"The NYSE 40-day breadth context could not be calculated from the current data feed.","behavior_explanation":"No contrarian or confirming interpretation is assigned while unavailable.","freshness_type":"UNAVAILABLE","last_updated":None,"validation_status":"RESEARCH_PENDING","error":errors["t2108"]}
