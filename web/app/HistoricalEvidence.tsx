...
export default function HistoricalEvidence({ evidence }: { evidence: CanonicalHistoricalEvidence | null }) {
  ...
  return <section className="card section-card historical-evidence-card">
    ...
    <details className="history-details">
      <summary><span><b>About this evidence</b><small>Validation source and sample definition</small></span><ChevronDown size={18} /></summary>
      <div className="history-details-body">
        <div className="evidence-method-grid">
          <div><b>Archived entry-timing validation</b><p>{retail.final_independent_reentry_episodes} independent retail opportunities are preserved in the canonical validation. The detailed frozen validator contains {final.count} cooldown-selected events. These are validation samples used to evaluate forward returns after RE-ENTRY signals.</p></div>
        </div>
      </div>
    </details>
