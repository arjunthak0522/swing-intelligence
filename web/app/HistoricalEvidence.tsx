...
function EpisodeDetails({ row }: { row: HistoricalEpisodeRow }) {
  ...
  return <div className="episode-expanded">
    <div className="episode-story">
      <b>What happened</b>
      <p>RE-ENTRY began on {dateLabel(row.start)} and remained the active state until the {dateLabel(row.next_state_date)} close, when the model changed to {state}. During that active period, SPY returned <strong>{pct(row.SPY_episode_return, 2)}</strong> and QQQ returned <strong>{pct(row.QQQ_episode_return, 2)}</strong>.</p>
    </div>
    ...
    <div className="episode-detail-columns">
      <div className="episode-detail-panel">
        ...
        <dl>
          ...
          <div><dt>State changed</dt><dd>{dateLabel(row.next_state_date)} → <strong>{state}</strong></dd></div>
          <div><dt>RE-ENTRY sessions</dt><dd>{row.reenter_sessions}</dd></div>
        </dl>
      </div>
      <div className="episode-detail-panel">
        <b>Signal context</b>
...
function EpisodePerformanceSummary({ ledger }: { ledger: HistoricalEpisodeLedger }) {
  ...
  return <div className="episode-performance-block">
    <div className="subsection-heading"><div><span className="kicker">WHILE RE-ENTRY WAS ACTIVE</span><h3>Performance through the close when the state changed</h3></div><span className="pill">{ledger.completed_episode_count} completed</span></div>
    <p className="history-note">Each reconstructed period starts at the first RE-ENTRY close and ends at the completed close where the model changes to WAIT or NO RE-ENTRY SETUP.</p>
...
function EpisodeLedger({ ledger }: { ledger: HistoricalEpisodeLedger | null }) {
  ...
  return <>
    ...
    <details className="history-details episode-ledger-details">
      <summary><span><b>View all {ledger.episode_count} reconstructed RE-ENTRY periods</b><small>What happened while RE-ENTRY remained the active state, newest first</small></span><ChevronDown size={18} /></summary>
      <div className="history-details-body">
        ...
        <div className="definition-box"><b>How to read this table</b><p>Each row begins at the first RE-ENTRY completed close and runs through the completed close where the state changes. The return columns measure that full active period.</p></div>
        <div className="episode-ledger-head"><span>RE-ENTRY began</span><span>State changed</span><span>New state</span><span>RE-ENTRY sessions</span><span>SPY period return</span><span>QQQ period return</span><span></span></div>
        <div className="episode-ledger">
          {rows.map((row, i) => <details className="episode-row" key={`${row.start}-${i}`}>
            <summary><span><b>{dateLabel(row.start)}</b></span><span>{dateLabel(row.next_state_date)}</span><span>{row.next_state_date ? <b>{String(row.next_state).replaceAll("_", " ")}</b> : <b>ACTIVE</b>}</span><span>{row.reenter_sessions}</span><strong>{pct(row.SPY_episode_return, 2)}</strong><strong>{pct(row.QQQ_episode_return, 2)}</strong><ChevronDown size={15} /></summary>
