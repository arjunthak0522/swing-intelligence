...
function EpisodeLedger({ ledger }: { ledger: HistoricalEpisodeLedger | null }) {
  ...
  return <>
    ...
    <details className="history-details episode-ledger-details" open>
      ...
      <div className="history-details-body">
        ...
        <div className="episode-ledger">
          {rows.map((row, i) => <details className="episode-row" key={`${row.start}-${i}`}>
            <summary><span><b>{dateLabel(row.start)}</b><small>→ {dateLabel(row.next_state_date)}</small></span><span>{row.reenter_sessions} {row.reenter_sessions === 1 ? "session" : "sessions"}</span><strong>{pct(row.SPY_episode_return, 2)}</strong><strong>{pct(row.QQQ_episode_return, 2)}</strong><ChevronDown size={15} /></summary>
