...
export default function CategorizedIndicatorBoard({ washout }: { washout: WashoutSnapshot }) {
  ...
  const categories: Category[] = [
    {
      ...
      rows: [
        row("S&P 500 Stocks Above 20-Day Average", pctPoint(v.SPXA20R), v.SPXA20R != null && v.SPXA20R <= 30 ? "OVERSOLD" : "NORMAL", "—", "INTRADAY / DELAYED", "DECISION INPUT", "SPXA20R"),
        row("U.S. Stocks Above 5-Day Average", pctPoint(v.MMFD), clean(v.MMFD_STATE || w.mmfd_live?.state), "—", w.mmfd_live?.timestamp_et ? "INTRADAY" : "UNAVAILABLE", "DECISION INPUT", "MMFD"),
        row("Nasdaq Breadth Momentum Strength", fmt(v.NASI_RSI), v.NASI_RSI != null && v.NASI_RSI < 30 ? "OVERSOLD" : "NORMAL", clean(v.NASI_DIRECTION), "INTRADAY / PROVISIONAL", "DECISION INPUT", "NASI+ RSI"),
        row("Nasdaq Breadth Fast Trend", fmt(v.NASI_EMA4), "REFERENCE", "—", "INTRADAY / PROVISIONAL", "SETUP CONTEXT", "NASI+ EMA4"),
        row("Nasdaq Breadth Slow Trend", fmt(v.NASI_EMA10), "REFERENCE", "—", "INTRADAY / PROVISIONAL", "SETUP CONTEXT", "NASI+ EMA10"),
    ...
    },
    {
      ...
      rows: [
        row("Rapid Breadth Improvement", boolState(fastNow.FAST_BREADTH_TURN ?? fastNow.fast_breadth_turn), boolState(fastNow.FAST_BREADTH_TURN ?? fastNow.fast_breadth_turn), boolDir(fastNow.FAST_BREADTH_TURN ?? fastNow.fast_breadth_turn), "CURRENT SNAPSHOT", "DERIVED TRIGGER", "Fast Breadth Turn"),
        row("Breadth Momentum Improvement", boolState(fastNow.MOMENTUM_TURN ?? fastNow.momentum_turn), boolState(fastNow.MOMENTUM_TURN ?? fastNow.momentum_turn), boolDir(fastNow.MOMENTUM_TURN ?? fastNow.momentum_turn), "CURRENT SNAPSHOT", "DERIVED TRIGGER", "Momentum Turn"),
        row("Buying-Volume Improvement", boolState(fastNow.NET_VOLUME_TURN ?? fastNow.net_volume_turn), boolState(fastNow.NET_VOLUME_TURN ?? fastNow.net_volume_turn), boolDir(fastNow.NET_VOLUME_TURN ?? fastNow.net_volume_turn), "CURRENT SNAPSHOT", "DERIVED TRIGGER", "Net Volume Turn"),
    ...
    },
    {
      ...
      rows: [
        row("NYSE Buying vs Selling Volume", fmt(v.NYUD), "VOLUME", "—", "INTRADAY / DELAYED", "FAST-FAMILY INPUT", "NYUD"),
        row("Nasdaq Buying vs Selling Volume", fmt(v.NAUD), "VOLUME", "—", "INTRADAY / DELAYED", "FAST-FAMILY INPUT", "NAUD"),
    ...
    },
    {
      ...
      rows: [
        ...
        row("Volatility-of-Volatility Recent Percentile", pctPoint(vvix.historical_percentile_2m ?? v.VVIX_PERCENTILE_2M), "PERCENTILE", clean(vvix.direction_vs_prior_close || v.VVIX_DIRECTION), "COMPLETED-HISTORY CONTEXT", "CONTEXT"),
        row("Live Downside Protection Premium Ratio", fmt(skew.live_proxy_ratio ?? v.SKEW_LIVE_PROXY_RATIO, 3), "TAIL RISK", clean(skew.direction_vs_prior_snapshot || v.SKEW_DIRECTION), clean(skew.freshness_state || skew.source_mode), "CONTEXT SUPPORT INPUT", "SKEW proxy"),
        row("Live Downside Protection Premium Spread", fmt(skew.live_proxy_vol_points ?? v.SKEW_LIVE_PROXY), "TAIL RISK", clean(skew.direction_vs_prior_snapshot || v.SKEW_DIRECTION), clean(skew.freshness_state || skew.source_mode), "CONTEXT", "SKEW vol spread"),
        row("Official Cboe Tail-Risk Index", fmt(skew.official_skew_latest_close ?? v.SKEW_OFFICIAL_CLOSE), "OFFICIAL CLOSE", clean(skew.official_skew_direction), "PRIOR CLOSE", "CONTEXT", "SKEW"),
        row("Nasdaq-100 Single-Stock Downside Protection Premium", fmt(ndxSkew.three_day_average ?? ndxSkew.raw_average, 3), clean(ndxSkew.state), clean(ndxSkew.direction), clean(ndxSkew.freshness_state || ndxSkew.freshness_type), "LEADING CONTEXT", "1M normalized put/call skew proxy · 3D avg"),
        row("Volatility Curve Shape", fmt(vol.current_ratio, 3), clean(vol.state), vol.supportive === true ? "SUPPORTIVE" : vol.supportive === false ? "NOT SUPPORTIVE" : "UNAVAILABLE", clean(vol.freshness_state || vol.freshness_type), "SECONDARY CONFIRMATION", "VIX / VIX3M"),
      ],
    },
    {
    ...
    },
    {
      ...
      rows: [
        ...
        row("Credit Risk Improvement", fmt(credit.hyg_lqd_ratio, 4), clean(credit.state), clean(credit.direction), clean(credit.freshness_state || credit.freshness_type), "LEADING CONTEXT", "HYG / LQD"),
      ],
    },
    {
      title: "CONTEXT TURNS",
      ...
      rows: [
        row("Short-Term Market Breadth Improving", boolState(context.MMFD_IMPROVING), boolState(context.MMFD_IMPROVING), boolDir(context.MMFD_IMPROVING), "CURRENT SNAPSHOT", "DERIVED CONTEXT INPUT", "MMFD"),
        row("Nasdaq Breadth Trend Turning Up", boolState(context.NASI_TURNING_UP), boolState(context.NASI_TURNING_UP), boolDir(context.NASI_TURNING_UP), "CURRENT SNAPSHOT", "DERIVED CONTEXT INPUT", "NASI"),
        row("Volatility-of-Volatility Easing", boolState(context.VVIX_EASING), boolState(context.VVIX_EASING), boolDir(context.VVIX_EASING), "CURRENT SNAPSHOT", "DERIVED CONTEXT INPUT", "VVIX"),
        row("Downside Tail-Risk Pricing Narrowing", boolState(context.SKEW_NARROWING), boolState(context.SKEW_NARROWING), boolDir(context.SKEW_NARROWING), "CURRENT SNAPSHOT", "DERIVED CONTEXT INPUT", "SKEW"),
    ...
    },
    {
      ...
      rows: [
        row("Strong Short-Term Market Breadth", boolState(extension.MMFD_STRONG), boolState(extension.MMFD_STRONG), boolDir(extension.MMFD_STRONG), "CURRENT SNAPSHOT", "CONTEXT ONLY", "MMFD"),
        row("Strong S&P 500 20-Day Breadth", boolState(extension.SPXA20R_STRONG), boolState(extension.SPXA20R_STRONG), boolDir(extension.SPXA20R_STRONG), "CURRENT SNAPSHOT", "CONTEXT ONLY", "SPXA20R"),
        row("Nasdaq Breadth Momentum Overbought", boolState(extension.NASI_OVERBOUGHT), boolState(extension.NASI_OVERBOUGHT), boolDir(extension.NASI_OVERBOUGHT), "CURRENT SNAPSHOT", "CONTEXT ONLY", "NASI"),
      ],
    },
    {
      title: "MARKET CONTEXT",
      description: "Where the S&P 500 and Nasdaq-100 ETFs are trading right now.",
      rows: [
  ...
  const meanings: Record<string, string> = {
    "S&P 500 Stocks Above 20-Day Average": "Shows the share of S&P 500 stocks still above their 20-day trend. A very low percentage means weakness is widespread enough to create an oversold re-entry setup.",
    "U.S. Stocks Above 5-Day Average": "Shows the share of U.S. stocks holding above their 5-day trend. A very low percentage means short-term participation is washed out and can activate the oversold setup.",
    "Nasdaq Breadth Momentum Strength": "Measures how stretched Nasdaq participation momentum is. Very low readings mean the internal market is washed out; a turn higher is the more important recovery signal.",
    "Nasdaq Breadth Fast Trend": "A faster view of Nasdaq participation that helps show whether market internals are starting to repair before the slower trend catches up.",
    "Nasdaq Breadth Slow Trend": "A steadier view of Nasdaq participation that helps show whether the broader internal trend is improving or deteriorating.",
    "NYSE Stocks Above 40-Day Average": "Shows how many NYSE stocks remain above their intermediate-term trend. Low readings mean weakness is broad, not isolated to a few stocks.",
    "Nasdaq Stocks Above 5-Day Average": "Shows how many Nasdaq stocks have reclaimed their 5-day average. It reacts quickly to an early internal recovery.",
    "Nasdaq Stocks Above 10-Day Average": "Shows how many Nasdaq stocks have reclaimed their 10-day average. It is steadier than the 5-day measure and helps confirm participation repair.",
    "Rapid Breadth Improvement": "Flags a fresh increase in the number of stocks participating after a washout. It is one of four fast reversal signals that can help trigger DEPLOY.",
    "Breadth Momentum Improvement": "Flags improving market participation momentum versus the prior snapshot. It is one of four fast reversal signals.",
    "Buying-Volume Improvement": "Flags a shift toward more trading volume in advancing stocks after heavy selling. It looks for buyers to return before price has fully recovered.",
    ...
    "Market Volatility": "Shows the market's expected near-term S&P 500 volatility. It provides fear context but does not directly change the RE-ENTRY decision.",
    "Volatility of Volatility": "Measures stress inside the volatility market itself. A falling reading can become an engine context signal that fear is easing.",
    "Volatility-of-Volatility Recent Percentile": "Places today's volatility-of-volatility reading against recent history so you can tell whether stress inside the volatility market is unusually high or low.",
    "Live Downside Protection Premium Ratio": "Compares the cost of S&P 500 downside protection with comparable upside options. A narrowing premium can become an engine context signal that crash fear is easing.",
    "Live Downside Protection Premium Spread": "Shows how many implied-volatility points investors are paying for S&P 500 downside protection versus comparable upside options.",
    ...
    "Nasdaq-100 Single-Stock Downside Protection Premium": "Measures how much extra option premium investors are paying for downside protection across individual Nasdaq-100 stocks. Lower readings mean less single-stock hedging demand. This is a free-data proxy, not Goldman's proprietary series.",
    "Volatility Curve Shape": "Compares near-term market volatility with three-month volatility. A normal upward-sloping curve suggests immediate stress is less acute, but this does not change DEPLOY.",
    "Equity Put/Call — Intraday": "Current same-session Cboe equity put/call activity. It is intraday and delayed, not the finalized daily equity put/call close.",
    "Equity Put/Call — Prior Official Close": "The most recent finalized daily equity put/call reading that was actually available before this session. A same-day finalized reading is never used.",
    ...
    "Total Put/Call": "Combines put and call activity across the options market to show the overall level of defensive positioning.",
    "Index Put/Call": "Shows defensive positioning in index options, which are often used by institutions for portfolio hedging.",
    "Risk-Appetite Broadening": "Checks whether investors are moving beyond mega-cap leaders into equal-weight stocks, small caps and credit risk.",
    "Equal Weight vs S&P 500": "Compares equal-weight S&P 500 performance with the standard cap-weighted S&P 500 ETF. Improvement suggests the rally is broadening beyond the largest stocks.",
    "Small Caps vs S&P 500": "Compares small-company stocks with the S&P 500. Improvement suggests investors are becoming more willing to own economically sensitive risk.",
    ...
    "Credit Risk Improvement": "Tracks whether lower-quality corporate bonds are beginning to outperform higher-quality bonds, an early cross-asset sign that investors are becoming more willing to take risk.",
    "Short-Term Market Breadth Improving": "Becomes active when the share of U.S. stocks above their 5-day average improves versus the prior snapshot. It is one of four context signals that can help qualify an early DEPLOY.",
    "Nasdaq Breadth Trend Turning Up": "Becomes active when broader Nasdaq participation starts rising from a weak level. It is one of four engine context signals.",
    "Volatility-of-Volatility Easing": "Becomes active when stress inside the volatility market falls, signaling that fear is beginning to calm. It can help qualify an early DEPLOY.",
    "Downside Tail-Risk Pricing Narrowing": "Becomes active when the premium for extreme downside protection narrows, signaling less demand for crash hedges. It can help qualify an early DEPLOY.",
    "Supportive Context Signals": "Counts how many of the four engine context turns are currently supportive.",
    "Strong Short-Term Market Breadth": "Flags that a large share of U.S. stocks are back above their 5-day average. It describes a mature or extended rebound and does not create or revoke DEPLOY.",
    "Strong S&P 500 20-Day Breadth": "Flags that a large share of S&P 500 stocks are above their 20-day average. It describes a broad, more mature recovery rather than a re-entry trigger.",
    "Nasdaq Breadth Momentum Overbought": "Flags unusually strong Nasdaq participation momentum. It describes an extended recovery and does not alter the re-entry trigger.",
    "S&P 500 ETF": "Current S&P 500 ETF price context so the internal signal can be viewed alongside the market's actual move.",
    "Nasdaq-100 ETF": "Current Nasdaq-100 ETF price context so the internal signal can be viewed alongside the market's actual move.",
