...
function nasiMeaning(v?:number|null,direction?:string|null){
  if(!finite(v))return"NASI RSI is unavailable.";
  ...
  return`${x.toFixed(1)} places this derived NASI RSI in the ${state} zone under the RE-ENTRY RSI interpretation framework. ${x<30?"The slower Nasdaq breadth structure is still deeply stretched.":x>70?"The slower breadth structure is extended rather than washed out.":"The slower breadth structure is in its middle range."} ${move}`;
}
...
function vvixMeaning(v?:number|null,direction?:string|null,pct2m?:number|null){
  if(!finite(v))return"VVIX is unavailable, so volatility-of-volatility stress cannot be assessed.";
  const x=Number(v),state=vvixState(v).toLowerCase(),d=(direction||"").toUpperCase();
  const move=d==="FALLING"?"VVIX is falling, so volatility anxiety is easing and supporting recovery confirmation.":d==="RISING"?"VVIX is rising, so volatility anxiety is increasing and not confirming the recovery yet.":"VVIX is broadly stable.";
...
function liveSkewMeaning(r?:number|null,direction?:string|null){
  if(!finite(r))return"Live SPX skew is unavailable, so immediate downside-hedging pressure cannot be assessed.";
  ...
  return`${x.toFixed(2)}x puts the 25-delta put-IV/call-IV proxy in the ${state} zone under RE-ENTRY proxy bands. ${x>=1.20?"Downside protection is extremely expensive versus comparable calls, showing stretched fear and a potential contrarian setup.":x>1.05?"Downside protection is more expensive than upside exposure, showing elevated hedging demand.":"Put and call implied volatility are broadly balanced."} ${move}`;
}
function officialSkewMeaning(v?:number|null,p?:number|null,direction?:string|null){
  if(!finite(v))return"Official Cboe SKEW is unavailable.";
  ...
  return`${x.toFixed(1)} is ${state} under RE-ENTRY's interpretation bands around the Cboe index.${pctText} High SKEW is fear context, not a standalone buy signal. ${move}`;
}
...
export default function ReentryDecisionDetails({washout}:{washout:WashoutSnapshot}){
 ...
 const contextRows=[["U.S. Stocks Above 5-Day Moving Average ($MMFD) improving",context.MMFD_IMPROVING,"NOT IMPROVING","Are more U.S. stocks getting back above their very short-term trend?"],["NASI RSI turning upward",context.NASI_TURNING_UP,"NOT TURNING UP","Is broader Nasdaq participation beginning to turn up from a weak level?"],["VVIX easing",context.VVIX_EASING,"NOT EASING","Is fear inside the volatility market itself starting to calm down?"],["SKEW narrowing",context.SKEW_NARROWING,"NOT NARROWING","Is demand for crash protection becoming less extreme?"]] as const;
 ...
 <MetricCard name="NYSE McClellan Oscillator" symbol="$NYMO · NYSE breadth momentum" value={signed(w.NYMO,1)} benchmark={historyBenchmark(hist.NYMO,"Established technician anchors: -100 oversold · 0 centerline · +100 overbought. More extreme magnitude should be interpreted with historical percentile/context rather than extra invented fixed bands.")} state={historyState(hist.NYMO,momentumState(w.NYMO))} direction={fast.MOMENTUM_TURN?"IMPROVING":"NO MOMENTUM TURN"} behavior="CONTRARIAN AT EXTREMES" behaviorExplanation="Deep negative readings can mark exhaustion, but the useful confirmation is momentum turning upward from the oversold/negative zone." freshness={fresh("DAILY CLOSE",hist.NYMO?.history_end_date||marketDate)} detail={historyDetail(hist.NYMO)} interpretation={momentumMeaning("NYSE McClellan",w.NYMO,fast.MOMENTUM_TURN)}/>
 <MetricCard name="Nasdaq McClellan Oscillator" symbol="$NAMO · Nasdaq breadth momentum" value={signed(w.NAMO,1)} benchmark={historyBenchmark(hist.NAMO,"Established technician anchors: -100 oversold · 0 centerline · +100 overbought. More extreme magnitude should be interpreted with historical percentile/context rather than extra invented fixed bands.")} state={historyState(hist.NAMO,momentumState(w.NAMO))} direction={fast.MOMENTUM_TURN?"IMPROVING":"NO MOMENTUM TURN"} behavior="CONTRARIAN AT EXTREMES" behaviorExplanation="Deeply negative Nasdaq breadth can be a contrarian setup. A turn higher is what converts that setup into recovery evidence." freshness={fresh("DAILY CLOSE",hist.NAMO?.history_end_date||marketDate)} detail={historyDetail(hist.NAMO)} interpretation={momentumMeaning("Nasdaq McClellan",w.NAMO,fast.MOMENTUM_TURN)}/>
 <MetricCard name="Nasdaq Summation Index RSI" symbol="NASI RSI · derived from $NASI" value={plain(w.NASI_RSI,1)} benchmark="RE-ENTRY RSI interpretation framework for this derived indicator: <30 oversold · <20 very oversold · <10 extreme · 30-70 middle range · >70 overbought · >90 extreme. These are RSI-style interpretation bands, not official NASI thresholds." state={nasiState(w.NASI_RSI)} direction={w.NASI_DIRECTION} directionTone={w.NASI_DIRECTION==="FALLING"?"bad-text":w.NASI_DIRECTION==="RISING"?"good-text":undefined} behavior="CONTRARIAN + CONFIRMING" behaviorExplanation="Oversold is contrarian context; a rising NASI RSI is confirmation that the broader Nasdaq recovery is becoming more durable." freshness={fresh("DAILY / PRIOR COMPLETED CLOSE",marketDate)} detail={finite(w.NASI_EMA10)?`EMA10 ${plain(w.NASI_EMA10,1)}`:undefined} interpretation={nasiMeaning(w.NASI_RSI,w.NASI_DIRECTION)}/>
 <MetricCard name="NYSE Advance/Decline Volume" symbol="$NYUD · NYSE A/D volume" value={signed(w.NYUD,1)} benchmark={historyBenchmark(hist.NYUD,"Defensible anchor: 0 is balance. Above 0 = advancing volume dominates; below 0 = declining volume dominates. Raw magnitude is feed-dependent, so extremes use empirical historical percentile once reliable - no fixed 250/1000/2500 bands.")} state={historyState(hist.NYUD,volumeBreadthState(w.NYUD))} direction={fast.NET_VOLUME_TURN?"IMPROVING":"NO VOLUME TURN"} behavior="CONFIRMING" behaviorExplanation="Positive/rising volume breadth supports the idea that the rebound has real participation behind it. Magnitude extremes are percentile-based, not hard-coded." freshness={fresh("INTRADAY",w.NYUD_TIMESTAMP_ET||snapshotTime)} detail={historyDetail(hist.NYUD)} interpretation={volumeMeaning("NYSE A/D volume",w.NYUD,fast.NET_VOLUME_TURN)}/>
 <MetricCard name="Nasdaq Advance/Decline Volume" symbol="$NAUD · Nasdaq A/D volume" value={signed(w.NAUD,1)} benchmark={historyBenchmark(hist.NAUD,"Defensible anchor: 0 is balance. Above 0 = advancing volume dominates; below 0 = declining volume dominates. Raw magnitude is feed-dependent, so extremes use empirical historical percentile once reliable - no fixed 250/1000/2500 bands.")} state={historyState(hist.NAUD,volumeBreadthState(w.NAUD))} direction={fast.NET_VOLUME_TURN?"IMPROVING":"NO VOLUME TURN"} behavior="CONFIRMING" behaviorExplanation="Positive/rising Nasdaq volume breadth supports a healthier recovery. Magnitude extremes are percentile-based, not hard-coded." freshness={fresh("INTRADAY",w.NAUD_TIMESTAMP_ET||snapshotTime)} detail={historyDetail(hist.NAUD)} interpretation={volumeMeaning("Nasdaq A/D volume",w.NAUD,fast.NET_VOLUME_TURN)}/>
