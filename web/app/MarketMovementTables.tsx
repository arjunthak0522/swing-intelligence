...
export default function MarketMovementTables({ snapshot, live }: { snapshot: Snapshot; live: LiveSnapshot | null }) {
  ...
  return <>
    ...
    <details className="card section-card market-collapsible">
      <summary className="market-collapsible-summary">
        <div className="section-heading"><div><span className="kicker">ALL SUBSECTORS</span><h2>Subsector Daily Moves &amp; Repair</h2><p className="section-intro">Today&apos;s movement first, with the 5-day move and recent pullback beside it.</p></div><div className="collapsible-status"><span className="pill">{subsectors.length} tracked</span><ChevronDown size={18} /></div></div>
      </summary>
      ...
      <style jsx>{`
        .market-collapsible{padding:0;overflow:hidden}.market-collapsible-summary{list-style:none;cursor:pointer;padding:22px 24px}.market-collapsible-summary::-webkit-details-marker{display:none}.market-collapsible-summary .section-heading{margin:0}.market-collapsible-summary .section-intro{margin:6px 0 0}.collapsible-status{display:flex;align-items:center;gap:10px}.collapsible-status svg{transition:transform .16s}.market-collapsible[open] .collapsible-status svg{transform:rotate(180deg)}.market-collapsible-body{border-top:1px solid var(--line);padding:20px 24px 24px}
      `}</style>
    </details>
  </>;
}