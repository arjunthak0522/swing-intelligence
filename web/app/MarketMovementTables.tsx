...
type LiveSnapshot = {
  quotes: Record<string, { change_pct: number | null; market_state?: string | null }>;
};
...
export default function MarketMovementTables({ snapshot, live }: { snapshot: Snapshot; live: LiveSnapshot | null }) {
  ...
  const movementLabel = regularSession ? "Today" : "1D";
  ...
  const subsectors = useMemo(() => Object.entries(snapshot.subsector_intelligence?.proxies || {}).map(([symbol, x]) => {
    ...
    const today = regularSession ? live?.quotes?.[symbol]?.change_pct ?? null : x.return_1d;
    return { symbol, x, state, today };
  }).sort((a, b) => {
    let av = 0; let bv = 0;
  ...
  }), [snapshot, live, regularSession, subsectorSort, subsectorDirection]);

  const cadenceNote = regularSession
    ? "Today uses the live intraday quote feed. 5D and Below 20D High remain completed-close context."
    : "Market closed: movement is labeled 1D rather than Today. Subsector 1D uses the completed-close snapshot; sector 1D uses the latest quote-session change when available.";

  return <>
    <section className="card section-card">
      <div className="section-heading"><div><span className="kicker">ALL 11 SECTORS</span><h2>Sector Moves &amp; Repair</h2></div><span className="pill">{sectors.length}/11 loaded</span></div>
      <p className="section-intro">{regularSession ? "Today’s move first" : "Latest 1-day move first"}, with recent pullback and repair context beside it. The default view ranks the strongest sectors by the first movement column.</p>
      <div className="sort-bar" aria-label="Sort sectors"><span>Sort by</span><SortButton active={sectorSort === "today"} direction={sectorDirection} onClick={() => toggleSectorSort("today")}>{movementLabel}</SortButton><SortButton active={sectorSort === "drawdown"} direction={sectorDirection} onClick={() => toggleSectorSort("drawdown")}>Below 20D High</SortButton><SortButton active={sectorSort === "status"} direction={sectorDirection} onClick={() => toggleSectorSort("status")}>Repair Status</SortButton></div>
      <div className="sector-table movement-table">
        <div className="sector-table-head sector-move-head"><span>Sector</span><span>{movementLabel}</span><span>Below 20D High</span><span>Subsectors 3%+ Down</span><span>Repair Status</span></div>
        {sectors.map((row) => <div className="sector-table-row sector-move-row" key={row.symbol}><div><b>{row.label}</b><small>{row.symbol}</small></div><strong className={numeric(row.today, 0) > 0 ? "good-text" : numeric(row.today, 0) < 0 ? "bad-text" : "muted"}>{pct(row.today)}</strong><span>{pct(row.drawdown)}</span><span>{pct(row.damageShare)}</span><span className={row.repairing ? "good-text" : "muted"}>{row.repairing ? "Repairing" : "Neutral"}</span></div>)}
      </div>
      <div className="notice">{cadenceNote} Sector 5D return is not currently published by the validated close snapshot, so it is intentionally not shown rather than inferred.</div>
    </section>

    <section className="card section-card">
      <div className="section-heading"><div><span className="kicker">ALL SUBSECTORS</span><h2>Subsector Moves &amp; Repair</h2></div><span className="pill">{subsectors.length} tracked</span></div>
      <p className="section-intro">{regularSession ? "Today’s move first" : "The completed-close 1D move first"}, with the 5-day move and recent pullback immediately beside it. Every tracked subsector remains visible.</p>
      <div className="sort-bar" aria-label="Sort subsectors"><span>Sort by</span><SortButton active={subsectorSort === "today"} direction={subsectorDirection} onClick={() => toggleSubsectorSort("today")}>{movementLabel}</SortButton><SortButton active={subsectorSort === "fiveDay"} direction={subsectorDirection} onClick={() => toggleSubsectorSort("fiveDay")}>5D</SortButton><SortButton active={subsectorSort === "drawdown"} direction={subsectorDirection} onClick={() => toggleSubsectorSort("drawdown")}>Below 20D High</SortButton><SortButton active={subsectorSort === "status"} direction={subsectorDirection} onClick={() => toggleSubsectorSort("status")}>Status</SortButton></div>
      <div className="subsector-head"><span>Subsector</span><span>{movementLabel}</span><span>5D</span><span>Below 20D High</span><span>Status</span><span></span></div>
      <div className="internal-list">
        {subsectors.map(({ symbol, x, state, today }) => <details key={symbol} className="internal-row movement-row"><summary><div className="name-wrap"><span className="state-dot" data-state={state.dot} /><div><b>{x.label} <span>({symbol})</span></b><small>{sectorNames[x.parent_sector] || x.parent_sector}</small></div></div><div className="movement-metrics"><strong className={numeric(today, 0) > 0 ? "good-text" : numeric(today, 0) < 0 ? "bad-text" : "muted"}>{pct(today)}</strong><span>{pct(x.return_5d)}</span><span>{pct(x.drawdown_20d)}</span><b className={state.cls}>{state.label}</b><ChevronRight size={17} /></div></summary><div className="detail-grid"><span>{movementLabel} <b>{pct(today)}</b></span><span>5D return <b>{pct(x.return_5d)}</b></span><span>Below 20D High <b>{pct(x.drawdown_20d)}</b></span><span>Below 60D high <b>{pct(x.drawdown_60d)}</b></span><span>vs SPY 20D <b>{pct(x.relative_strength_20d_vs_spy)}</b></span><span>vs {x.parent_sector} 20D <b>{pct(x.relative_strength_20d_vs_parent)}</b></span></div><p className="detail-copy">{x.repairing ? `${x.label} is repairing after a meaningful reset. This is constructive context, not an independent re-entry trigger.` : state.label === "NEUTRAL" ? `${x.label} is not materially damaged on the 20-day measure and is not currently in repair mode.` : `${x.label} remains in a reset or correction. The engine tracks whether this weakness begins to stabilize and broaden into repair.`}</p></details>)}
      </div>
      <div className="notice">{cadenceNote}</div>
    </section>
  </>;
}
