...
type LiveSnapshot = { quotes: Record<string, { change_pct: number | null; market_state?: string | null }> };
...
const levelPct = (value?: number | null, digits = 0) => typeof value === "number" && Number.isFinite(value)
  ? `${(value * 100).toFixed(digits)}%`
  : "-";
const numeric = (value: number | null | undefined, fallback = Number.NEGATIVE_INFINITY) => typeof value === "number" && Number.isFinite(value) ? value : fallback;
...
export default function MarketMovementTables({ snapshot, live }: { snapshot: Snapshot; live: LiveSnapshot | null }) {
  ...
  return <>
    <section className="card section-card">
      ...
      <p className="section-intro">{periodLabel} movement first, with pullback and repair context beside it. Sector context is secondary to the RE-ENTRY decision.</p>
      <div className="sort-bar" aria-label="Sort sectors"><span>Sort by</span><SortButton active={sectorSort === "today"} direction={sectorDirection} onClick={() => toggleSectorSort("today")}>{periodLabel}</SortButton><SortButton active={sectorSort === "drawdown"} direction={sectorDirection} onClick={() => toggleSectorSort("drawdown")}>Below 20D high</SortButton><SortButton active={sectorSort === "status"} direction={sectorDirection} onClick={() => toggleSectorSort("status")}>Repair status</SortButton></div>
      <div className="sector-table movement-table">
        <div className="sector-table-head sector-move-head"><span>Sector</span><span>{periodLabel}</span><span>Below 20D high</span><span>Subsectors 3%+ down</span><span>Status</span></div>
