...
function tone(state?: string, supportive?: boolean) {
  ...
  if (s.includes("ACUTE") || s.includes("DEFENSIVE") || s.includes("HIGH_FEAR")) return "secondary-bad";
...
export default function SecondaryConfirmation({ washout }: { washout: any }) {
  ...
  return <section className="card section-card secondary-confirmation">
    <style>{`
      ...
      .secondary-title{font-size:10px;font-weight:850}.secondary-state{display:inline-block;margin:8px 0 9px;padding:4px 7px;border-radius:999px;border:1px solid currentColor;font-size:8px;font-weight:900;letter-spacing:.045em}.secondary-good .secondary-state{color:var(--green)}.secondary-warn .secondary-state{color:var(--amber)}.secondary-bad .secondary-state{color:var(--red)}
      .secondary-readout{font-size:12px;font-weight:800;line-height:1.35}.secondary-plain{margin:9px 0 0;font-size:10px;line-height:1.5;color:#3f443d}.secondary-plain b{display:block;font-size:8px;letter-spacing:.08em;color:var(--muted);margin-bottom:2px}.secondary-benchmark{margin-top:9px;padding:8px;border-radius:9px;background:#f0eee8;font-size:9px;line-height:1.45;color:#53574f}.secondary-validation{margin-top:8px;font-size:8px;font-weight:850;letter-spacing:.04em;color:var(--muted)}.secondary-note{margin:6px 0 0;font-size:9px;line-height:1.42;color:var(--muted)}
    ...
    <div className="secondary-grid">{entries.map(([key,family]) => <div key={key} className={`secondary-card ${tone(family.state,family.supportive)}`}>
      ...
      <span className="secondary-state">{(family.state || "UNAVAILABLE").replaceAll("_"," ")}</span>
    ...
    <p className="secondary-foot">{overlay.interpretation} These signals are deliberately descriptive until their incremental historical value is proven.</p>
  </section>;
}
